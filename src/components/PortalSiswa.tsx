...
export default function PortalSiswa({
  ...
  return (
    <div className={`min-h-screen ${isLoggedIn ? 'bg-gray-50' : 'bg-gradient-to-br from-blue-600 via-blue-500 to-indigo-600'} flex flex-col font-sans`} id="student-portal-root">
      
      {/* HEADER SECTION (HIDDEN IN ACTIVE TEST TO MAXIMIZE FOCUS AREA) */}
      {!activeExam && (
        <header className={`${isLoggedIn ? 'bg-white text-slate-800 border-b border-slate-200/80 shadow-xs' : 'bg-transparent text-white border-b border-white/10'} print:hidden`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`h-9 w-9 ${isLoggedIn ? 'bg-slate-900 text-white' : 'bg-white text-blue-600'} rounded-lg flex items-center justify-center font-bold text-base shadow-sm`}>
                5
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs ${isLoggedIn ? 'text-slate-400' : 'text-blue-200'} font-semibold uppercase tracking-wider`}>Portal Siswa</span>
                  <span className={`text-xs ${isLoggedIn ? 'text-slate-300' : 'text-white/20'}`}>/</span>
                  <span className={`text-xs font-bold ${isLoggedIn ? 'text-blue-600' : 'text-white'} uppercase tracking-wider`}>Ruang Ujian</span>
                </div>
                <h1 className={`text-base font-extrabold tracking-tight ${isLoggedIn ? 'text-slate-900' : 'text-white'} font-sans`}>
                  CAT_SMPN5LR
                </h1>
              </div>
            </div>
            ...
            {isLoggedIn && (
              <button
                ...
                className="px-3 py-1.5 bg-slate-100 hover:bg-rose-50 border border-slate-200 text-slate-600 hover:text-red-600 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
              >
            ...
            {!isLoggedIn && (
              <button
                ...
                className="px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
              >
      ...
      {!isLoggedIn && (
        <div className="flex-1 flex items-center justify-center px-4 py-12" id="student-login-view">
          <motion.div 
          ...
          >
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 font-sans tracking-tight">Ujian Masuk Siswa</h2>
              <p className="mt-1 text-sm text-gray-500">Pilih kelas dan isi nama lengkap sesuai dokumen absensi.</p>
            </div>
            ...
            <form className="mt-8 space-y-5" onSubmit={handleLogin}>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  {classes.map(cls => (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                />
              </div>
              ...
              <button
                ...
                type="submit"
                className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-md"
              >
                <LogIn size={18} />
      ...
      {isLoggedIn && !activeExam && (
        <div className="flex-1 max-w-4xl w-full mx-auto px-4 py-10" id="student-dashboard-view">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Selamat Datang, {activeSession.studentName}!</h2>
                <p className="text-xs text-gray-500">Anggota Kelas Terdaftar: <span className="font-bold text-blue-600">Kelas {activeSession.className}</span></p>
              </div>
            </div>
            
            <div className="bg-slate-100 px-4 py-2 rounded-lg text-xs font-semibold text-slate-700">
          ...
          {activeExamsForClass.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border border-gray-200 text-gray-400">
              ...
              <p className="font-semibold text-gray-600">Tidak Ada Ujian Aktif</p>
              <p className="text-xs mt-1">Saat ini belum ada jadwal ujian aktif untuk Kelas {activeSession.className}. Silakan hubungi guru pengawas Anda.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeExamsForClass.map(exam => {
                ...
                return (
                  <div key={exam.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-xs flex flex-col justify-between">
                    <div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-gray-500">
                        <p>Kriteria KKM: <span className="font-bold text-gray-800">{exam.kkm}</span></p>
                        <p>Banyak Pertanyaan: <span className="font-bold text-gray-800">{exam.questions.length} Soal</span></p>
                      </div>
                    </div>

                    <div className="mt-6 pt-4 border-t border-gray-100">
                      {isLocked ? (
                        <div className="w-full text-center py-2 bg-red-100 text-red-800 text-xs font-bold rounded-lg flex items-center justify-center gap-1">
                          <Lock size={14} />
                          <span>DIKUNCI KARENA KECURANGAN</span>
                        </div>
                      ) : isDone ? (
                        <div className="w-full text-center py-2 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-lg flex items-center justify-center gap-1">
                          <CheckCircle size={14} />
      ...
      {activeExam && activeSession && (
        <div className="flex-1 flex flex-col lg:flex-row h-screen overflow-hidden bg-slate-900 text-white" id="cat-test-room-root">
          ...
          {showCheatWarning && (
            <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50">
              <motion.div 
                ...
                className="max-w-md w-full bg-red-950 border-2 border-red-500 rounded-2xl p-8 text-center text-red-100 space-y-6 shadow-2xl"
              >
                <div className="mx-auto h-20 w-20 bg-red-900 text-red-500 rounded-full flex items-center justify-center animate-bounce">
                  <ShieldAlert size={48} />
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-extrabold uppercase tracking-wide">PERINGATAN KERAS!</h3>
                  <p className="text-xs text-red-300 font-semibold font-mono bg-red-900/40 p-2 rounded border border-red-800">
                ...
                <div className="p-3 bg-red-900/20 text-xs rounded border border-red-900 leading-relaxed italic text-red-300">
                  Jangan mencoba berpindah tab browser, membuka chat, meminimalkan aplikasi, atau membuka tab lain. Pelanggaran berikutnya akan otomatis <span className="font-bold underline text-red-100">mengunci paksa lembar ujian</span> dan memberikan nilai 0.
                </div>
                <button
                  id="acknowledge-cheat"
                  onClick={() => setShowCheatWarning(false)}
                  className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-xs uppercase tracking-wider cursor-pointer shadow-lg"
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-950 text-center">
              <div className="max-w-md space-y-6">
                <div className={`mx-auto h-16 w-16 rounded-full flex items-center justify-center mb-4 ${
                  activeSession.isCheated ? 'bg-red-900/30 text-red-500' : 'bg-emerald-900/30 text-emerald-500'
                }`}>
                  {activeSession.isCheated ? <Lock size={36} /> : <CheckCircle size={36} />}
                </div>
                <div>
                ...
                <button
                  ...
                  className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold rounded-lg cursor-pointer"
                >
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <aside className="w-full lg:w-72 bg-slate-950 p-5 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-slate-800 flex-shrink-0 select-none overflow-y-auto">
                      <div>
                        {/* Student mini info */}
                        <div className="p-3 bg-slate-900 rounded-lg border border-slate-800 mb-6">
                          <p className="text-[10px] text-slate-400 uppercase tracking-widest leading-none font-bold">PESERTA UJIAN</p>
                          <p className="font-bold text-sm text-slate-100 truncate mt-1">{activeSession.studentName}</p>
                          <p className="text-xs text-slate-400 mt-0.5">Kelas: {activeSession.className}</p>
                        </div>

                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">NAVIGASI NOMOR SOAL</p>
                        ...
                        <div className="grid grid-cols-5 gap-2">
                          {activeExam.questions.map((qst, idx) => {
                            ...
                            if (isActive) {
                              btnClass = 'bg-blue-600 text-white ring-2 ring-blue-400 border-transparent';
                            } else if (isDoubt) {
                              btnClass = 'bg-amber-500 text-slate-950 border-transparent font-bold';
                            } else if (isAnswered) {
                              btnClass = 'bg-emerald-600 text-white border-transparent';
                            }
                    ...
                    <main className="flex-1 flex flex-col justify-between overflow-hidden">
                      ...
                      <div className="px-6 py-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between gap-4 flex-shrink-0">
                        <div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2 bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800">
                            ...
                            <span className="font-mono font-bold text-sm tracking-widest text-slate-100" id="cat-countdown-timer">
                              {timeString}
                            </span>
                          </div>
                          ...
                          <button
                            ...
                            className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-extrabold rounded-lg cursor-pointer flex items-center gap-1.5 shadow-sm transition-colors"
                          >
                            <CheckCircle size={14} />
                            <span>SELESAI</span>
                          </button>
